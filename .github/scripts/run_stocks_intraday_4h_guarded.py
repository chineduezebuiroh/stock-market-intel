...
from core.intraday_cadence import is_stocks_4h_opportunity
...
def run_profile() -> None:
    ...
    def notify_repair_telemetry(warn_count: int = 50, warn_pct: float = 0.15) -> None:
        ...
        df["bars_repaired"] = pd.to_numeric(
            df["bars_repaired"], errors="coerce"
        ).fillna(0)
        df["repair_pct"] = df["bars_repaired"] / df["bars_total"].replace(0, pd.NA)

        bad = df[
            (df["bars_repaired"] >= warn_count) | (df["repair_pct"] >= warn_pct)
        ].copy()
    ...
    results += run_combo_health(
        combos=["stocks_d_4hdw_shortlist"], universe_csv="shortlist_stocks.csv"
    )
...
def main() -> None:
    ...
    if not is_stocks_4h_opportunity(now):
        print(f"[INFO] {now} NY is not in a Stocks 4h opportunity. Skipping.")
        sys.exit(0)
